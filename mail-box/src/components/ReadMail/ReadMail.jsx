...
function ReadMail({ mailId }) {
  ...
  try{ 
    await fetch (`https://mail-box-1c3dd-default-rtdb.firebaseio.com/mail/${id}.json`,
  {method:"DELETE"}
  )}catch(err){
    console.log(err)
  }
      ...
      {mail ? (
        <div className="w-full bg-white shadow-md p-6 rounded-lg">
          <div className="flex justify-between items-center mb-4">
            ...
            <button onClick={()=>handleDelete(mail.id)} className="text-gray-600 hover:text-gray-800" >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 24 24"
                fill="currentColor"
